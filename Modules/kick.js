/*
    * CommandTitle: "Kick"
    * CommandSubtitle: "Kicks a specified user!"
...
module.exports = async (incomingMessage, commandArguments) => {
    ...
    const commandAction = "kick";
    const { guild: messageGuild, author: messageAuthor, client: botClient } = incomingMessage;
    const userPermissions = checkUserPermission(messageAuthor, messageGuild, "KICK_MEMBERS");
    const botPermissions = checkBotPermission(messageGuild, "KICK_MEMBERS");

    if (messageGuild && userPermissions && botPermissions) {
        incomingMessage.reply("kick!");
    } else {
